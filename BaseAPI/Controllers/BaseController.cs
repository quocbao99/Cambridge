        ...
        where F : BaseSearch, new()
    {
        ...
        public virtual async Task<AppDomainResult> DeleteItem(Guid id)
        {
            ...
            if (entity != null)
            {
                return new AppDomainResult()
                {
                    ...
                    ResultMessage = ApiMessage.SuccessfulDelete,
                    Success = true
                };
            }
            else
        ...
        public List<Permission> ListPermission()
        {
            ...
            foreach (Assembly assem in assems)
            {
                ...
                typeof(ControllerBase).IsAssignableFrom(type) && !type.IsAbstract && type.GetCustomAttributes(typeof(NotInPermissionsAttribute)).Select(x => x.GetType().Name).FirstOrDefault() != typeof(NotInPermissionsAttribute).Name)
                  .Select(e => new Permission()
                  {
                      ...
                      PermissionActions = (from i in e.GetMembers().Where(x => (x.Module.Name == "API.dll" || x.Module.Name == "BaseAPI.dll") 
                                                && x.Name != ".ctor"
                                                && x.Name != "GetPermission"
                                                && x.Name != "ListPermission"
                                                && x.GetCustomAttributes(typeof(NonActionAttribute)).Select(x => x.GetType().Name).FirstOrDefault() != typeof(NonActionAttribute).Name
                                                && x.GetCustomAttributes(typeof(NotInPermissionsAttribute)).Select(x => x.GetType().Name).FirstOrDefault() != typeof(NotInPermissionsAttribute).Name)
                  ...
                  .Where(e => e.Controller != "Auth" && e.Controller != "Menu" && e.Controller != "Catalogue")
        ...
        public async Task<Permission> GetPermission(string controllerName)
        {
            return await Task.Run(() =>
            {
                ...
                foreach (Assembly assem in assems)
                {
                    ...
                    typeof(ControllerBase).IsAssignableFrom(type) && !type.IsAbstract && type.Name.Replace("Controller", string.Empty) == controllerName && type.GetCustomAttributes(typeof(NotInPermissionsAttribute)).Select(x => x.GetType().Name).FirstOrDefault() != typeof(NotInPermissionsAttribute).Name)
                      .Select(e => new Permission()
                      {
                          Controller = e.Name.Replace("Controller", string.Empty),
                          Description = string.Format("{0}", ReflectionUtilities.GetClassDescription(e)).Replace("Controller", string.Empty),
